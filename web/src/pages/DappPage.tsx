...
export function DappPage() {
  ...
  const premium = unitPremium * amount;
  const payout = amount * USDC;
  ...
  const handleTrade = async () => {
    ...
    try {
      ...
      const betHash = await writeContractAsync({
        ...
        abi: binaryOptionAbi,
        functionName: "buy",
        args: [selected.id, side, amount, premium]
      });
      setTxHash(betHash);
      setTxStage("confirming");
        ...
        <section className="dapp-hero">
          ...
          <div className="desk-status-panel">
            <div className="desk-status-topline">
              <span className={contractsConfigured ? "desk-dot live" : "desk-dot"} />
              <strong>{contractsConfigured ? "Контракты подключены" : "Preview режим"}</strong>
            </div>
