...
const isArcTestnet = chainId === 5_042_002;
export const explorerUrl =
  import.meta.env.VITE_EXPLORER_URL ??
  (isArcTestnet ? "https://testnet.arcscan.app" : "");

export const chronicleChain = defineChain({
  id: chainId,
  name: import.meta.env.VITE_CHAIN_NAME ?? "Chronicle Local",
  nativeCurrency: {
    name: isArcTestnet ? "USDC" : "Ether",
    symbol: isArcTestnet ? "USDC" : "ETH",
    decimals: 18
  },
  rpcUrls: {
    default: {
  ...
  blockExplorers: explorerUrl
    ? {
        default: {
          name: isArcTestnet ? "ArcScan" : "Explorer",
          url: explorerUrl
        }
      }
    : undefined
